...
aba_recipe <- recipe(age ~ type + longest_shell + diameter + height+whole_weight + shucked_weight+ viscera_weight + shell_weight,  data= aba_train) %>%
 step_dummy_multi_choice(all_nominal_predictors()) %>%
   ...
   step_interact(terms = ~ longestshell:diameter) %>%     step_interact(terms=  ~ shucked_weight:shell_weight) %>%
...
#6
``` {r }
abaF <-data.frame(longest_shell = 0.50, diameter = 0.10, height = 0.30, whole_weight = 4, shucked_weight = 1, viscera_weight = 2, shell_weight = 1)
